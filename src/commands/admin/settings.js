...
module.exports = class SettingsCommand extends Command {
  ...
  run(message, args) {
    ...
    const cheeseRate = `\`${row.cheese_rate}\``;
    let cheeseIgnoreChannels = [];
    if (row.cheese_ignore_channels) {
    ...
    switch (setting) {
      ...
      case 'cheeses':
        embed
          ...
          .addField('Rate', cheeseRate, true)
          .addField('Ignoring', cheeseIgnoreChannels, true);
        return message.channel.send(embed);
    }
    if (setting)
    ...
    embed
      ...
      .addField('Cheese', '`3` settings', true)
