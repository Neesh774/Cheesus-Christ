...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Cheesus Christ has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const CheesusChristColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (CheesusChristColor) await guild.me.roles.add(CheesusChristColor);
  } catch (err) {
    client.logger.error(err.message);
  }
